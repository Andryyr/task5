...
import com.github.Andryyr.task5.model.DBHelper;
...
public class DBTest {
    ...
    private void check(String description, String srcUrl, boolean result) throws InterruptedException {
        helper.check(description, srcUrl).blockingSubscribe(result ? asserterTrue : asserterFalse);
    }

    private void add(String description, String srcUrl, boolean result) throws InterruptedException {
        helper.add(description, srcUrl).blockingSubscribe(result ? asserterTrue : asserterFalse);
    }

    private void del(String description, String srcUrl, boolean result) throws InterruptedException {
        helper.delete(description, srcUrl).blockingSubscribe(result ? asserterTrue : asserterFalse);
    }
